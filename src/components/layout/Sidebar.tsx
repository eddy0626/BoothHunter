...
export default function Sidebar() {
  ...
  return (
    <aside className="w-14 lg:w-56 bg-white border-r border-gray-200 flex flex-col transition-[width] duration-200">
      {/* Logo */}
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center gap-2">
          <Store className="w-6 h-6 text-indigo-600 shrink-0" />
          <h1 className="text-lg font-bold text-gray-900 hidden lg:block">{t.appName}</h1>
        </div>
        <p className="text-xs text-gray-500 mt-1 hidden lg:block">Booth.pm {t.nav.search}</p>
      </div>
      ...
      <nav className="flex-1 p-2 overflow-y-auto" aria-label={t.a11y.mainNav}>
        ...
        {/* VRChat Category Tree — hidden when collapsed */}
